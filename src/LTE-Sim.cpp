...
int main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		if (strcmp(argv[1], "SingleCell") == 0)
		{
			...
			double maxDelayVoIP = atof(argv[9]);
			double maxDelayVideo = atof(argv[10]);
			double maxDelayWeb = atof(argv[11]);
			double maxDelayCBR = atof(argv[12]);
			int video_bit_rate = atoi(argv[13]);
			SingleCellWithoutInterference(radius, nbUE, pVoIP, pVideo, pBE, pCBR, sched_type, 30, maxDelayVoIP, maxDelayVideo, maxDelayWeb, maxDelayCBR, video_bit_rate);
		}
